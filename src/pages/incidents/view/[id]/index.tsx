...
import { useIncidentFindFirst, useRoqClient } from 'lib/roq';
...
function IncidentViewPage() {
  ...
  const queryParams = useMemo<{ include: { school: boolean; user: boolean } }>(
    () =>
  ...
  const { data, error, isLoading } = useIncidentFindFirst(queryParams, {}, { disabled: !id });
            ...
            <FormWrapper wrapperProps={{ border: 'none', gap: 3, p: 0 }}>
              <Flex alignItems="center" w="full" justifyContent={'space-between'}>
                ...
                {hasAccess('incident', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                  <NextLink href={`/incidents/edit/${id}`} passHref legacyBehavior>
                    <Button
              ...
              >
                <FormListItem label="Incident Type" text={data?.incident_type} />

                <FormListItem
                  label="Incident Date"
                  text={data?.incident_date ? format(data?.incident_date, 'dd-MM-yyyy') : ''}
                />
