...
import { useRoqClient, useIncidentFindManyWithCount } from 'lib/roq';
import { convertQueryToPrismaUtil } from 'lib/utils';

type ColumnType = ColumnDef<IncidentInterface, unknown>;
...
export function IncidentListPage(props: IncidentListPageProps) {
      ...
      convertQueryToPrismaUtil(
        {
          ...
          searchTermKeys: ['incident_type.contains', 'description.contains'],
          ...params.filters,
        },
        'incident',
      ),
  ...
  const { data, error, isLoading, mutate } = useIncidentFindManyWithCount({
  ...
  const handleDelete = async (id: string) => {
    setDeleteError(null);
    try {
      await roqClient.incident.delete({ where: { id } });
      await mutate();
    } catch (error) {
  ...
  const handleView = (row: IncidentInterface) => {
    if (hasAccess('incident', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/incidents/view/${row.id}`);
    }
  };

  const columns: ColumnType[] = [
    { id: 'incident_type', header: 'Incident Type', accessorKey: 'incident_type' },
    {
      id: 'incident_date',
      header: 'Incident Date',
      accessorKey: 'incident_date',
      cell: ({ row: { original: record } }: any) =>
        record?.incident_date ? format(record?.incident_date, 'dd-MM-yyyy') : '',
    },
      ...
      ? {
          ...
          cell: ({ row: { original: record } }: any) => (
            <Flex justifyContent="flex-end">
              <NextLink href={`/incidents/view/${record.id}`} passHref legacyBehavior>
                <Button
