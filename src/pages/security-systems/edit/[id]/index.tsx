...
import { useRoqClient, useSecuritySystemFindFirst } from 'lib/roq';
...
function SecuritySystemEditPage() {
  ...
  const { data, error, isLoading, mutate } = useSecuritySystemFindFirst(queryParams, {}, { disabled: !id });
  const [formError, setFormError] = useState(null);

  const handleSubmit = async (values: SecuritySystemInterface, { resetForm }: FormikHelpers<any>) => {
    setFormError(null);
    try {
      const updated = await roqClient.security_system.update({
        data: values as RoqTypes.security_system,
        where: {
          id,
        },
      });
