...
import { useRoqPlatformUserProfiles, useAccessLogCount, useIncidentCount, useSecuritySystemCount } from 'lib/roq';

import { IncidentListPage as ShowCaseList } from 'pages/incidents';

function DashboardPage() {
  const { session } = useSession();

  const accessLogStat = useAccessLogCount();

  const incidentStat = useIncidentCount();

  const securitySystemStat = useSecuritySystemCount();

  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      title: 'Access Logs',
      result: accessLogStat?.data,
      isLoading: accessLogStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Incidents',
      result: incidentStat?.data,
      isLoading: incidentStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Security Systems',
      result: securitySystemStat?.data,
      isLoading: securitySystemStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
          ...
          <Stack direction="column" spacing={8} flex={{ base: 1, md: 7 }}>
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                ...
                <NextLink href={`incidents`} passHref legacyBehavior>
                  <Button
